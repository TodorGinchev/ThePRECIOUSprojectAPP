...
public class WristbandFoodRecognition extends Activity  {
	...
	//Frame size is 479 lines and 106 rows
	private static final int FRAME_COLS = 106; 
	private static final int FRAME_ROWS = 479; 
	private static final int FRAME_SIZE = FRAME_ROWS*FRAME_COLS; 
	...
	public void getPhoto(){	
		Mat inputMat = new Mat(FRAME_ROWS, FRAME_COLS, CvType.CV_8U);
		try{
			File ext_storage = Environment.getExternalStorageDirectory();
			String extPath = ext_storage.getPath();
			BufferedReader br = new BufferedReader( new FileReader(extPath+"/precious/BTdata2.txt"));

			char[] buffer = new char [FRAME_SIZE+1];
			Log.i("TAG","Attempting to read " + FRAME_SIZE + " bytes");

			if (br.read(buffer, 0, FRAME_SIZE) != FRAME_SIZE){
			...
			for (int i=0;i<FRAME_ROWS;i++){
				for(int j=0;j<FRAME_COLS;j++){
					inputMat.put(i, j, buffer[FRAME_COLS*i+j]);
				}
			}
